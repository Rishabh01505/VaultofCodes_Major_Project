  ...
  $(function () {
    ...
    function initOwlCarousel(c) {
      ...
      for (var j = 0; j < values.length; j++) {
        ...
        for (var k = j; k >= -1; k--) {
          ...
          if (
            !responsive[values[j]]["autoWidth"] &&
            responsive[values[j]]["autoWidth"] !== 0 &&
            c.attr("data" + aliaces[k] + "auto-width")
          ) {
            responsive[values[j]]["autoWidth"] =
              k < 0 ? false : c.attr("data" + aliaces[k] + "auto-width");
          }
        }
      }
    ...
    function applyAnimation(target, params) {
      ...
      function start() {
        ...
        animeObj = anime(merge(defaultParams, params.animationStart));
      }

      function end() {
        if (animeObj) animeObj.pause();
        animeObj = anime(merge(defaultParams, params.animationEnd));
      }
    ...
    function initBootstrapTooltip(tooltipPlacement) {
      ...
      for (var i = 0; i < plugins.bootstrapTooltip.length; i++) {
        ...
        if (window.innerWidth < 576) {
          $tooltip.tooltip({
            customClass: $tooltip.attr("data-class")
              ? $tooltip.attr("data-class")
              : "",
            placement: "bottom",
          });
        } else {
      ...
      for (var i = 0; i < plugins.bootstrapTabs.length; i++) {
        ...
        if ($bootstrapTabsItem.attr("data-transition-state") === "true") {
          $bootstrapTabsItem.on("hide.bs.tab", function (event) {
            document
              .querySelectorAll(event.target.getAttribute("href"))[0]
      ...
      for (var z = 0; z < plugins.rdNavbar.length; z++) {
        ...
        for (i = j = 0, len = values.length; j < len; i = ++j) {
          ...
          if ($rdNavbar.attr("data" + aliaces[i] + "anchor-nav-offset")) {
            responsiveNavbar[values[i]]["anchorNavOffset"] = $rdNavbar.attr(
              "data" + aliaces[i] + "anchor-nav-offset"
            );
          }
    ...
    function updAnchOffset() {
      var relatives = document.querySelectorAll(this.dParams.anchor.offsetRef),
        dY = 0;
    ...
    function updStuckOffset() {
      this.dParams.stuck.offset = document
        .querySelector(this.dParams.stuck.offsetRef)
        ...
        node.addEventListener(
          ...
          function () {
            if (this.currentAnchor && !isInView(this.currentAnchor)) {
              var nav = $(this.node).find(".navigation"),
                offset =
                  $(this.currentAnchor).parent().offset().left +
                  nav.scrollLeft();
